...
test.describe('Форматирование текста в карточках', () => {
    test('1. text-formatter.js: applyFormatting генерирует правильный HTML', async ({ page }) => {
    ...
    test('2. CSS: проверка правил для .flashcard-content span', async ({ page }) => {
        await page.goto(BASE_URL);
    ...
    test('3. DOM: проверка структуры .flashcard-front .flashcard-content', async ({ page }) => {
    ...
    test('4. Редактор: форматирование применяется в редакторе', async ({ page }) => {
        await page.goto(`${BASE_URL}/#/stats`);
        await page.waitForTimeout(500);
        await page.click('button:has-text("Ежедневная сессия")');
        await page.waitForTimeout(1000);
    ...
    test('5. Карточка: форматирование отображается на лицевой стороне', async ({ page }) => {
        await page.goto(`${BASE_URL}/#/stats`);
        await page.waitForTimeout(500);
        await page.click('button:has-text("Ежедневная сессия")');
        await page.waitForTimeout(1000);
        ...
        console.log('Стили span на ВОПРОСЕ:', spanStyles);
        expect(spanStyles).toBeTruthy();
        expect(spanStyles.display).toBe('inline');
    });

    test('6. Карточка: форматирование отображается на обратной стороне', async ({ page }) => {
        await page.goto(`${BASE_URL}/#/stats`);
        await page.waitForTimeout(500);
        await page.click('button:has-text("Ежедневная сессия")');
        await page.waitForTimeout(1000);
        ...
        // Выделяем текст в ответе
        await page.focus('#edit-answer-editor');
        await page.evaluate(() => {
            const editor = document.getElementById('edit-answer-editor');
            const range = document.createRange();
            const selection = window.getSelection();
            const textNode = editor.childNodes[0];
            if (textNode && textNode.textContent.length >= 5) {
        ...
        // Применяем цвет
        await page.click('.format-text-color-btn[data-color="#7FCDCD"]');
        await page.waitForTimeout(500);
        ...
        // Проверяем стили span на ответе
